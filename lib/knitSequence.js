...
var KnitSequence = function() {
    ...
    */
     ...
     */
    this.generate = function(outFileName, desc = "", position = "Keep", machine = undefined, autoRack = true, halfGauge = false, castOff = true, backend = undefined) {
        ...
        } else if(backend === wrapper.SINTRAL) {
            fixationArea.left = 1;
            fixationArea.right = leftmost - 1;
        } else {
            throw Error("fixation area not defined for backend '" + wrapper.backendToString(backend) + "'");
        }
