...
var KnitSequence = function() {
    ...
    */
     ...
     * @param {Boolean} castonForTube
     */
    this.generate = function(outFileName, desc = "", position = undefined, machine = undefined, autoRack = undefined, halfGauge = undefined, castOff = undefined, backend = undefined, castonBeds = undefined, castonForTube = undefined) {
        ...
        if(castonBeds === undefined)
            castonBeds = (castonForTube ? FRONT | BACK : FRONT);
        if(castonForTube && castonBeds !== (FRONT | BACK))
            console.warn("castonForTube flag is set but not both beds are cast on -- flag will have no effect");
                        ...
                        if(!ci.carrier.isIn) {
                            ...
                            if(!numActiveCarriers) {
                                if(course.ops.length) {
                                    ...
                                    if(castonBeds & BACK)
                                        b.push('b');
                                    kw.castOn(ci.carrier, course.leftPos, course.leftPos + course.ops.length - 1, STITCHNUMBER_CASTON, b, castonForTube);
                            ...
                            while(n !== end) {
                                ...
                                switch (ops[i]) {
                                    ...
                                    case 'X':
                                        ...
                                        break;
                                    default:
                                        console.warn("WARNING: unknown needle operation '" + ops[i] + "'");
                                        break;
                                }
