...
const SPEEDNUMBER_CASTON  = 12;
const SPEEDNUMBER_CASTOFF = 13;
            ...
            switch (cmd) {
                case 'n': //needle operations (knit, tuck, miss, etc.)
                    ...
                    if(ys.length === 1) {
                        ...
                        let il = String(carriageCounter).length;
                        let cl = String(courseNr + 1).length;
                        
                        let d = dirToString(course.dir);
                        console.log('N:  ' + ' '.repeat(coml - il) + (carriageCounter) + ': ' + ' '.repeat(maxIdLen - y.length) + y  + ' ' + d + ' ' + ' '.repeat(coul - cl) + '(' + (courseNr + 1) + '): ' +  ' '.repeat(course.leftPos - 1) + course.ops);
                        ...
                        ys.forEach(function(y, i, arr) {
                            ...
                            let d = dirToString(maps[y].courses[courseCntr[y]].dir);
                            if(d !== dirToString(course.dir))
                                console.warn("WARNING: plating directions disagree!");
                            ...
                            if(i === 0)
                                str += 'N:  ' + ' '.repeat(coml - il) + (carriageCounter) + ': ';
                            else
                                str += '    ' + ' '.repeat(coml) + '  ';
        ...
        commands.forEach(function(command, cidx) {
            ...
            switch (cmd) {
                case 'n': //needle operations (knit, tuck, miss, etc.)
                    ...
                    if(ys.length === 1) {
                        ...
                        if(!ci.carrier.isIn) {
                            ...
                            if(!numActiveCarriers) {
                                if(course.ops.length) {
                                    ...
                                    if(castonBeds & BACK)
                                        b.push('b');
                                    kw.castOn(ci.carrier, course.leftPos, course.leftPos + course.ops.length - 1, SPEEDNUMBER_CASTON, STITCHNUMBER_CASTON, b, tube);
                        ...
                        if(first !== undefined && dir !== NODIR) {
                            ...
                            let end = (dir === RIGHT ? r + 1 : l - 1);
                            let n = start;
                            let i = (dir === RIGHT ? first : last);
                            ...
                            while(n !== end) {
                                ...
                                switch (course.ops[i]) {
                                    ...
                                    case 'b':
                                        ...
                                        if(currentRacking !== 0.25)
                                            throw Error("racking has to be 0.25 for needle operation 'b' (knit front, knit back) to work");
                                    ...
                                    case 'B':
                                        //TODO: not sure about what happens (and what *SHOULD* happen!) when bed is racked
                                        if(currentRacking !== 0.25)
                                            throw Error("racking has to be 0.25 for needle operation 'B' (tuck front, tuck back) to work");
                                    ...
                                    case 'y':
                                        ...
                                        kw.split(dir, 'f', n, n +  Math.round(currentRacking), c);
                                        restoreRacking();
                                        break;
                                    case 'Y':
                                        alignRacking('split(back)');
                                        kw.split(dir, 'b', n, n +  Math.round(currentRacking), c);
                                        restoreRacking();
                                        break;
                                    case 'x':
                                        if(currentRacking !== 0.25)
                                            throw Error("racking has to be 0.25 for needle operation 'x' (knit front, tuck back) to work");
                                    ...
                                    case 'X':
                                        if(currentRacking !== 0.25)
                                            throw Error("racking has to be 0.25 for needle operation 'X' (tuck front, knit back) to work");
                        ...
                        if(ci.carrier.courseCntr === m.courses.length) {
                            ...
                            if(isEndOfKnit) {
                                if(castOff) {
                                    ...
                                    if(range)
                                        kw.castOff(ci.carrier, range.l, range.r, SPEEDNUMBER_CASTOFF, STITCHNUMBER_CASTOFF, b, CHAINLOOPS_CASTOFF, tube);
                                } else {
                        ...
                        //TODO: check for some more consistency for plating here
                        for(let i = 1; i < courseList.length; i++)
                            if(courseList[i].dir !== dir)
                                console.warn("plating directions disagree (" + ciList[i].carrier.name + dirToString(courseList[0].dir) + " vs. " + ciList[0].carrier.name + dirToString(dir) + " in course #" + carriageCounter + ")");
                            ...
                            while(n !== end) {
                                ...
                                switch (ops[i]) {
                                    ...
                                    case 'B':
                                        //TODO: not sure about what happens (and what *SHOULD* happen!) when bed is racked
                                        if(currentRacking !== 0.25)
                                            throw Error("racking has to be 0.25 for needle operation 'x' (tuck front, tuck back) to work");
                ...
                case 'x': //needle transfer
                    ...
                    let intRacking = Math.round(currentRacking);
                    ...
                    for(let i = 0; i < tr.ops.length; i++, nt++) {
                        switch(tr.ops[i]) {
                            ...
                            case 'x':
                                ...
                                kw.xfer('f', nt, nt + intRacking);
                                break;
                            case 'X':
                                //transfer back-to-front
                                kw.xfer('b', nt, nt + intRacking);
                                break;
                            case 's':
                                //transfer front-to-backslider
                                kw.xfer('f', nt, nt + intRacking, false, true);
                                break;
                            case 'S':
                                //transfer back-to-frontslider
                                kw.xfer('b', nt, nt + intRacking, false, true);
                                break;
                            case 'r':
                                //transfer frontslider-to-back
                                kw.xfer('f', nt, nt + intRacking, true, false);
                                break;
                            case 'R':
                                //transfer backslider-to-front
                                kw.xfer('b', nt, nt + intRacking, true, false);
                                break;
                            default:
